...
static int MakeFakeLicense(char *license_path, SceNpDrmLicense *license_buf) {
  ...
  snprintf(path, sizeof(path) - 1, "ux0:nonpdrm/%s/%s", p+1, rif_name);
...
static int ksceNpDrmGetRifInfoPatched(SceNpDrmLicense *license_buf, int license_size, int mode,
                                      char *content_id, uint64_t *aid, uint16_t *license_version, uint8_t *license_flags,
                                      uint32_t *flags, uint32_t *sku_flag, uint64_t *start_time, uint64_t *expiration_time, uint64_t *flags2) {
  int res = TAI_CONTINUE(int, ksceNpDrmGetRifInfoRef, license_buf, license_size, mode,
                                                      content_id, aid, license_version, license_flags,
                                                      flags, sku_flag, start_time, expiration_time, flags2);
...
static int ksceNpDrmGetRifVitaKeyPatched(SceNpDrmLicense *license_buf, uint8_t *klicensee, uint32_t *flags, uint32_t *sku_flag,
                                         uint64_t *start_time, uint64_t *expiration_time) {
  int res = TAI_CONTINUE(int, ksceNpDrmGetRifVitaKeyRef, license_buf, klicensee, flags, sku_flag, start_time, expiration_time);
...
int module_start(SceSize args, void *argp) {
  ...
  hooks[n_hooks++] = taiHookFunctionExportForKernel(KERNEL_PID, &ksceNpDrmGetRifInfoRef,           "SceNpDrm", 0xD84DC44A, 0xDB406EAE, ksceNpDrmGetRifInfoPatched);
  hooks[n_hooks++] = taiHookFunctionExportForKernel(KERNEL_PID, &ksceNpDrmGetRifVitaKeyRef,        "SceNpDrm", 0xD84DC44A, 0x723322B5, ksceNpDrmGetRifVitaKeyPatched);
  hooks[n_hooks++] = taiHookFunctionExportForKernel(KERNEL_PID, &ksceNpDrmGetRifNameRef,           "SceNpDrm", 0xD84DC44A, 0xDF62F3B8, ksceNpDrmGetRifNamePatched);
  hooks[n_hooks++] = taiHookFunctionExportForKernel(KERNEL_PID, &ksceNpDrmGetRifNameForInstallRef, "SceNpDrm", 0xD84DC44A, 0x17573133, ksceNpDrmGetRifNameForInstallPatched);
